...
vi.mock('swiper/react', () => ({
  Swiper: ({ children, onSwiper, ...props }: { children: ReactNode; onSwiper?: (swiper: SwiperInstance) => void }) => {
...
it('enables keyboard navigation and lets a thumbnail select its matching slide', async () => {
  ...
  expect(swiperProps[0]).toMatchObject({ keyboard: { enabled: true }, tabIndex: 0 });
  await user.click(screen.getByRole('button', { name: 'Show Road Bear image 2' }));
